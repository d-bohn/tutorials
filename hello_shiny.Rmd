...
versions of shiny, these had to be seperate, consisting of a `UI.R` and a
...
## Running the app
After you save the document as `app.R`, and if you are using RStuido, you should
...
first walkthrough I am going to break them down into their seperate components.
...
There are also a number of HTML "tags" that are built into shiny for text formating
...
but some common ones include:
    ...
    - `tags$div()` adds a division, usually to be formated in a specific way
    - `tags$p()` inserts a paragrah of text
    - `tags$i()` italicizes text
    - `tags$img()` creates an image

**Caveat**: Notice that the tags need to be supersceeded by `tags$`, but
...
take a look at the [raw code here]()
